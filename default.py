### ############################################################################################################
###	#	
### # Project: 			#		KissAnime (.me,.com,.to) - Addon by The Highway 2013-2015.
### # Author: 			#		The Highway
### # Version:			#		vX.X.X
...
def listItems(section=_default_section_,url='',startPage='1',numOfPages='1',genre='',year='',stitle='',season='',episode='',html='',chck=''): # List: Movies or TV Shows
	...
	#html=messupText(html,_html=True,_ende=True,_a=False,Slashes=False)
	##html=messupText(html,_html=True,_ende=False,_a=False,Slashes=False,_UTF8=True)
	...
	s='<td title=\'(.*?)\'\s*[/]*>\s*<a(?:\s+class="a'+ps('common_word')+'")?\s+href="(/'+ps('common_word')+'/.+?)"\s*>\s*(.+?)\s*</a>\s*(.*?)\s*</td>\s*<td>\s*(.*?)\s*</td>\s*</tr'; 
	...
	if (iitems is not None):
		...
		#for item_url,tInfo,name,imInfo,LInfo in iitems:
		for tInfo,item_url,name,imInfo,LInfo in iitems:
			tInfo=messupText(tInfo,_html=True,_ende=True,_a=False,Slashes=False)
			name=messupText(name,_html=True,_ende=True,_a=False,Slashes=False)
			imInfo=messupText(imInfo,_html=True,_ende=True,_a=False,Slashes=False)
			LInfo=messupText(LInfo,_html=True,_ende=True,_a=False,Slashes=False)
			...
			try: r=get_database_1st('SELECT * FROM visited WHERE (episode == "" AND title == "%s" AND url LIKE "%s")' % (name,_domain_url+"/%")); 
			except: pass
			if r:
				if len(r) > 0:
			...
			r2=False; 
			#try: r=get_database_1st('SELECT * FROM shows WHERE url == "%s"'%item_url); 
			try: r=get_database_1st('SELECT * FROM shows WHERE url == "%s"'%messupText(item_url,_html=True,_ende=False,_a=False,Slashes=False)); 
			except: pass
			if r:
				if len(r) > 0: 
					if r[0]==item_url:
			...
			## ### ## 
				...
				deb('title',labs[u'title']); 
				deb('url',item_url); 
				deb('img',img); deb('fanart',fimg)
				#deb('plot',labs['plot']); 
			## ### ## 
			if ('</a>' in LInfo):
				deb('LInfo',LInfo)
			...
			if UseLabelFormat==True:
				...
				labs[u'title']=messupText(labs['title'],_html=True,_ende=True,_a=False,Slashes=False)
			...
			if (tfalse(addst("Notyetaired"))==False) and (LInfo=='Not yet aired'): deb(LInfo,name)
				...
				try: _addon.add_directory(pars,labs,img=img,fanart=fimg,contextmenu_items=contextMenuItems,total_items=ItemCount)
					...
					debob(['could not display item',labs['title']])
					pass
...
def listEpisodes(section,url,img='',showtitle='',season=''): #_param['img']
	...
	debob(['length of html',str(len(html))]); 
	#debob(html); 
	if len(showtitle)==0:
	...
	if (len(fimg)==0) or ('icon.png' in fimg.lower()):
		#fimg=_artFanart
		fimg=img
	showtitle=messupText(showtitle,_html=True,_ende=True,_a=False,Slashes=False)
	...
	try: 
		...
		#do_database(['INSERT OR REPLACE INTO visited (url,title,episode,timestampyear,timestampmonth,timestampday,img,fanart) VALUES ("%s","%s","%s","%s","%s","%s","%s","%s")' % (  url,showtitle,"",str(datetime.date.today().year),str(datetime.date.today().month),str(datetime.date.today().day),img,fimg  )])
		do_database(['INSERT OR REPLACE INTO visited (url,title,episode,timestampyear,timestampmonth,timestampday,img,fanart) VALUES ("%s","%s","%s","%s","%s","%s","%s","%s")' % (  messupText(url,_html=True,_ende=True,_a=False,Slashes=False),showtitle,"",str(datetime.date.today().year),str(datetime.date.today().month),str(datetime.date.today().day),img,fimg  )])
	except: pass
	## ### ## 
	...
	sDB='SELECT %s FROM shows WHERE (url == "%s")' % (tab1rows,messupText(url,_html=True,_ende=True,_a=False,Slashes=False)); debob(sDB); 
	try: r=get_database_1st(sDB); 
	except: pass
	iFound=False; imdbid=addpr("imdbid",""); plot=addpr("plot","")
	if len(imdbid)==0: imdbid='0'
	...
	if r:
		...
		if len(r) > 0:
			...
			if r[0]==messupText(url,_html=True,_ende=True,_a=False,Slashes=False): iFound=True
	if iFound==False:
		try:
			...
			#try:    
			sDB+=' VALUES ("%s","%s","","%s","%s","%s","%s","%s","%s","%s")' % (  str(messupText(url,_html=True,_ende=True ,_a=False,Slashes=False)),str(showtitle),str(img),str(fimg),str(imdbid),str(plot),str(datetime.date.today().year),str(datetime.date.today().month),str(datetime.date.today().day)  ); 
			#except: sDB+=' VALUES ("%s","%s","","%s","%s","%s","%s","%s","%s","%s")' % (  str(messupText(url,_html=True,_ende=False,_a=False,Slashes=False)),str(showtitle),str(img),str(fimg),str(imdbid),str(plot),str(datetime.date.today().year),str(datetime.date.today().month),str(datetime.date.today().day)  ); 
			debob(""+sDB); 
			do_database([sDB]); 
			imdb_id=''
			ShowPlot=''
	...
	if len(ShowID) > 0:
		try:
			...
			sDB=['UPDATE shows SET showid = "%s" WHERE (url == "%s")' % (  ShowID,messupText(url,_html=True,_ende=True,_a=False,Slashes=False) )]
			debob(sDB); 
			do_database(sDB); 
		except: pass
	...
	#s='<img src="../../Content/images/bullet.png"\s*/>\s*<a href="(/'+ps('common_word')+'/.+?)"\s*>\s*<b>(.+?)</b>\s*</a>\s*<div class="clear" style="height:5px">\s*</div>'
	s='<img src="../../Content/images/\D+.png"\s*/>\s*<a href="(/'+ps('common_word')+'/.+?)"\s*>\s*(?:<b>)?\s*(.+?)\s*(?:</b>)?\s*</a>\s*<div class="clear" style="height:5px">\s*</div'
	...
	s='<tr>\s*<td>\s*<a\s*.*?href="(/'+ps('common_word')+'/.+?\?id=[0-9]+)"\s*title="\D*\s*'+ps('common_word2')+' .+?"\s*>\s*(.+?)\s*</a>\s*</td>\s*<td>\s*([0-9/]*)\s*</td'
	try:		episodes=re.compile(s).findall(nolines(html).replace('</tr>','</tr\n\r>'))
	except:	episodes=''
	#debob(episodes); 
	...
	## ### ## 
	if (len(img)==0) or ('icon.png' in img.lower()):
		#img=re.compile('<div class="barTitle">\s*Cover</div>\s*<div class="barContent">\s*<div class="arrow-general">\s*(?:&nbsp;\s*)?</div>\s*<div style="text-align: center">\s*<img width="\d+px" height="\d+px" src="(.+?)"/>').findall(nolines(html).replace('&nbsp;',' ').replace('\n','').replace('\r','').replace('\a','').replace('\t','').replace('<div class="clear2">','<div class="clear2"\n\r\a>').replace('<div class="rightBox">','<div class="rightBox"\n\r\a>'))[0]
	...
	if tfalse(addst("show-ep-status","false"))==True:
		...
		deb("Status",Status); 
		try: _addon.add_directory({'mode':'System.Exec','url':url},{'title':cFL('Status: ','green')+cFL(str(Status),'lime')+"[CR]"+cFL(showtitle,'deepskyblue')},img=showImg,fanart=showFanart,is_folder=True)
		except:
			try: 
				showtitle=messupText(showtitle,_html=True,_ende=True,_a=False,Slashes=False)
				_addon.add_directory({'mode':'System.Exec','url':url},{'title':cFL('Status: ','green')+cFL(str(Status),'lime')+"[CR]"+cFL(showtitle,'deepskyblue')},img=showImg,fanart=showFanart,is_folder=True)
			except:
				try: _addon.add_directory({'mode':'System.Exec','url':url},{'title':cFL('Status: ','green')+cFL(str(Status),'lime')+"[CR]"+cFL('[ERROR]','deepskyblue')},img=showImg,fanart=showFanart,is_folder=True)
				except: pass
	#
	# Date Aired
	if tfalse(addst("show-ep-dateaired","false"))==True:
		...
		try: _addon.add_directory({'mode':'System.Exec','url':url},{'title':cFL('Date Aired: ','green')+cFL(str(showDateAired),'lime')},img=showImg,fanart=showFanart,is_folder=True)
		except: pass
	...
	if tfalse(addst("next-ep-countdown","false"))==True:
		...
		s44ND='new Date(new Date().getTime() + '
		
		if s44+s44ND in html: 
			debob(['found',s44+s44ND])
			##try: 
			#s44='\$\("\#nextEpisodeCountDown"\)\.countdown\(new Date\(new Date\(\)\.getTime\(\)\s*\+\s*(\d+)\s*\)\s*,'; #'%-d day%!d '+'%H:%M:%S'
	...
	if ItemCount > 0: 
		...
		debob(['episodes',episodes])
		for ep_url,ep_name,ep_date in episodes:
			ep_name=messupText(ep_name,_html=True,_ende=True,_a=False,Slashes=False)
			ep_date=messupText(ep_date,_html=True,_ende=True,_a=False,Slashes=False)
			
			debob(ep_name); 
			ep_name_original=''+ep_name; 
			...
			#r=get_database_1st('SELECT * FROM visited WHERE (episode == "%s" AND title == "%s")' % (epnameo,showtitle)); 
			#try: r=get_database_1st('SELECT * FROM visited WHERE (url == "%s")' % (ep_url)); 
			#try: r=get_database_1st('SELECT * FROM visited WHERE (url == "%s")' % (messupText(ep_url,_html=True,_ende=False,_a=False,Slashes=False))); 
			...
			if (EnableEpisodeMeta==True) and (EnableMeta==True) and (vtype=='episode') and (len(imdb_id) > 2):
				...
				except: labs={'title':stitle,'imdb_id':imdb_id,'season':s,'episode':e,'episodetitle':ep_name}
				try: 
			...
			ep_url=messupText(ep_url,_html=True,_ende=True,_a=False,Slashes=False)
	...
	if tfalse(addst("show-ep-views","false"))==True:
		...
		deb("Views",Views); 
		try: _addon.add_directory({'mode':'System.Exec','url':url},{'title':cFL('Views: ','green')+cFL(str(Views),'yellow')+"[CR]"+cFL(showtitle,'deepskyblue')},img=showImg,fanart=showFanart,is_folder=True)
		except:
			try: _addon.add_directory({'mode':'System.Exec','url':url},{'title':cFL('Views: ','green')+cFL(str(Views),'yellow')+"[CR]"+cFL('[ERROR]','deepskyblue')},img=showImg,fanart=showFanart,is_folder=True)
			except: pass
	# Plot
...
def ListAdvertisements(section=_default_section_,html="",pageUrl="",url=""):
	html=nolines(html)
	try:    Advertisements=re.compile('<div id="divFloat((?:Left|Right)?)" style="position: absolute; visibility:visible">\s*<a target="_blank" href="(\D+://.+?)">\s*<img border="0" src="(\D+://kissanime\.\D+/ads/.+?)"/>\s*</a>\s*</div').findall(html.replace('</div>','</div\n\r\a>')); ### , re.MULTILINE | re.IGNORECASE | re.DOTALL
...
def Menu_2():
	...
	_addon.add_directory({'mode':'System.Exec','url':'http://kissanime.to/'},{'title': cFL_('KissAnime.to',ps('cFL_color3'))},is_folder=True,fanart=_artFanart,img='http://kissanime.to/Content/images/logo.png'); 
	...
	_addon.add_directory({'mode':'System.Exec','url':'http://kissanime.to/AdvanceSearch'},{'title': cFL_('Advanced Search',ps('cFL_color3'))},is_folder=True,fanart=_artFanart,img='http://kissanime.to/Content/images/logo.png'); 
	...
	_addon.add_directory({'mode':'System.Exec','url':'http://forum.kissanime.com/'},{'title': cFL_('KA Forum',ps('cFL_color3'))},is_folder=True,fanart=_artFanart,img='http://forum.kissanime.com/logo1.png'); 
	_addon.add_directory({'mode':'System.Exec','url':'http://kissanime.to/Message/FAQ'},{'title': cFL_('KA F.A.Q.',ps('cFL_color3'))},is_folder=True,fanart=_artFanart,img='http://kissanime.to/Content/images/logo.png'); 
...
def Menu_MainMenu(): #The Main Menu
	...
	#
#	url='http://tinyurl.com/o52ar6c'
#	html=CheckUrlForRedirect(url,headers={})
#	#html=nURL(url,'redirect')
#	#html=net2.http_HEAD2(url,headers={}).geturl()
#	html=net2.http_HEAD(url,headers={}).getLastHeader("Location").getValue()
#	html=net2.http_HEAD(url,headers={}).geturl()
#	html=net2.http_HEAD(url,'redirect',{})
#	html=urllib2.urlopen(url).geturl()
#	#html=net2.http_HEAD(url).content #.getheader("location",""); 
#	#html=urllib2.urlopen(url).info().headers
#	#html=urllib2.urlopen(url).info().items()
#	#html=urllib2.urlopen(url).info().dict
#	debob("+++"); debob(url); debob("---"); debob(html); debob("==="); 
	...
	if tfalse(addst("splash-screen"))==True: 
		...
		splash.do_My_Splash('http://kissanime.to/Content/images/logo.png',5); 
...
def check_mode(mode=''):
	...
	if ('kissanime.com/' in uRL) and (not 'forum' in uRL):
		uRL=uRL.replace('kissanime.com/','kissanime.to/')
	elif ('kissanime.me/' in uRL): # and (not 'forum' in uRL):
		uRL=uRL.replace('kissanime.me/','kissanime.to/')
	#####
	if (mode=='') or (mode=='main') or (mode=='MainMenu'): Menu_MainMenu()
	...
	elif (mode=='PlayTrailer'): 					PlayTrailer(uRL, _param['title'], _param['year'], _param['img'])
	elif (mode=='BrowseUrl'): 						
		url=uRL
	...
	elif (mode=='BrowseAZ'): 							Menu_BrowseByAZ(_param['section'],uRL)
	elif (mode=='SelectAZ'): 							Select_AZ(uRL)
	elif (mode=='SelectSort'): 						Select_Sort(uRL,_param['title'])
	elif (mode=='SelectGenre'): 					Select_Genre(uRL)
	...
	elif (mode=='GetUpcoming'): 					listUpcoming(_param['section'], uRL, addpr('pageno'), addpr('pagecount'), addpr('genre'), addpr('year'), addpr('title'))
	elif (mode=='GetTitles'): 						listItems(_param['section'], uRL, _param['pageno'], _param['pagecount'], _param['genre'], _param['year'], _param['title'])
	elif (mode=='GetTitlesUpcoming'): 		listItems_Upcoming(_param['section'], uRL, _param['pageno'], _param['pagecount'], _param['genre'], _param['year'], _param['title'])
	elif (mode=='GetTitlesLatest'): 			listItems(_param['section'], uRL, _param['pageno'], _param['pagecount'], _param['genre'], _param['year'], _param['title'], chck=ps('LI.tv.latest.check'))
	elif (mode=='GetTitlesLatestWatched'): listItems(_param['section'],uRL, _param['pageno'], _param['pagecount'], _param['genre'], _param['year'], _param['title'], chck=ps('LI.tv.latest.watched.check'))
	elif (mode=='GetTitlesPopular'): 			listItems(_param['section'], uRL, _param['pageno'], _param['pagecount'], _param['genre'], _param['year'], _param['title'], chck=ps('LI.tv.popular.all.check'))
	elif (mode=='GetTitlesHDPopular'): 		listItems(_param['section'], uRL, _param['pageno'], _param['pagecount'], _param['genre'], _param['year'], _param['title'], chck=ps('LI.movies.popular.hd.check'))
	elif (mode=='GetTitlesOtherPopular'): listItems(_param['section'], uRL, _param['pageno'], _param['pagecount'], _param['genre'], _param['year'], _param['title'], chck=ps('LI.movies.popular.other.check'))
	elif (mode=='GetTitlesNewPopular'): 	listItems(_param['section'], uRL, _param['pageno'], _param['pagecount'], _param['genre'], _param['year'], _param['title'], chck=ps('LI.movies.popular.new.check'))
	elif (mode=='GetLinks'): 							lLs('325273032032525032503252352052305923'); listLinks(_param['section'], uRL, showtitle=_param['showtitle'], showyear=_param['showyear'],epnameo=addpr('epnameo'),eptitle=addpr('title'))
	elif (mode=='GetEpisodes'): 					listEpisodes(_param['section'], uRL, _param['img'], _param['title'], _param['season'])
	elif (mode=='TextBoxFile'): 					TextBox2().load_file(uRL,_param['title']); eod()
	elif (mode=='TextBoxUrl'):  					TextBox2().load_url( uRL,_param['title']); eod()
	...
	elif (mode=='Bookmarks'):  						Bookmarks(_param['section'],uRL,addpr('urlr'),_param['title'])
	elif (mode=='listBookmarks'):  				listBookmarks(_param['section'],uRL,addpr('cat'))
	...
	elif (mode=='LibrarySaveMovie'):  		Library_SaveTo_Movies(uRL,_param['img'],_param['showtitle'],_param['showyear'])
	elif (mode=='LibrarySaveTV'):  				Library_SaveTo_TV(_param['section'], uRL,_param['img'],_param['showtitle'],_param['showyear'],_param['country'],_param['season'],_param['episode'],_param['episodetitle'])
	elif (mode=='LibrarySaveEpisode'):  	Library_SaveTo_Episode(uRL,_param['img'],_param['title'],_param['showyear'],_param['country'],_param['season'],_param['episode'],_param['episodetitle'])
	elif (mode=='PlayLibrary'): 					PlayLibrary(_param['section'], uRL, showtitle=_param['showtitle'], showyear=_param['showyear'])
	elif (mode=='Download'): 							print _param; DownloadRequest(_param['section'], uRL,_param['img'],_param['studio']); eod()
	elif (mode=='DownloadStop'): 					DownloadStop(); eod()
	#elif (mode=='LatestThreads'): 				News_LatestThreads(uRL,_param['title'])
	elif (mode=='GetLatestSearches'): 		listLatestSearches(_param['section'],uRL)
	elif (mode=='UsersShowProfileAccountInfo'): UsersShowPersonInfo(mode, _param['section'],uRL)
	elif (mode=='ChangeFanartList'):			ChangeFanartList(_param['section'],_param['subfav'],uRL,_param['fanart'],_param['img'],_param['studio'])
	elif (mode=='ChangeFanartUpdate'):		ChangeFanartUpdate(_param['section'],_param['subfav'],uRL,_param['title'])
		...
		##					
		#refresh_meta(addpr('video_type',''),addpr('title',''),addpr('imdb_id',''),addpr('alt_id',''),addpr('year',''))
		tEStTitle,tEStID,tEStLabs=refresh_meta(addpr('video_type',''),addpr('title',''),addpr('imdb_id',''),addpr('alt_id',''),addpr('year',''))
		if len(tEStTitle)+len(tEStID)==0: return
		try:    tEStLabsThumb=tEStLabs['cover_url']
		except: tEStLabsThumb=''
		try:    tEStLabsFanart=tEStLabs['backdrop_url']
		except: tEStLabsFanart=''
		tEStItemUrl=uRL
		r2=False; 
		#try: r=get_database_1st('SELECT * FROM shows WHERE url == "%s"'%item_url); 
		try: r=get_database_1st('SELECT * FROM shows WHERE url == "%s"'%messupText(tEStItemUrl,_html=True,_ende=False,_a=False,Slashes=False)); 
		except: pass
		if r:
			if len(r) > 0: 
				if r[0]==tEStItemUrl:
		...
		if r2==True:
			...
			#	except: imdbid="0"
			#if len(imdbid)==0: imdbid="0"
			try: tEStLabsTitle=tEStLabs['title']
			except: 
				try: tEStLabsTitle=tEStLabs['Title']
				except: tEStLabsTitle=""
			tEStTitle=tEStLabsTitle
			
			try: PloT=tEStLabs['plot']
			except: PloT=""
			
			#item_url=messupText(item_url,_html=True,_ende=True,_a=False,Slashes=False)
			
			if r2==False: ## ONLY-IF show was NOT found in SqlData. ##
					aAa1='INSERT OR REPLACE INTO shows (url,title,year,img,fanart,imdbnum,plot,timestampyear,timestampmonth,timestampday) '
					#aAa1+='VALUES ("%s","%s","%s","%s","%s","%s","%s","%s","%s","%s")' % (  str(tEStItemUrl),str(tEStTitle),str(""),str(tEStLabsThumb),str(tEStLabsFanart),str(tEStID),urllib.quote_plus(PloT),str(datetime.date.today().year),str(datetime.date.today().month),str(datetime.date.today().day)  )
					aAa1+='VALUES ("%s","%s","%s","%s","%s","%s","%s","%s","%s","%s")' % (  str(tEStItemUrl),str(r[1]),str(""),str(tEStLabsThumb),str(tEStLabsFanart),str(tEStID),urllib.quote_plus(PloT),str(datetime.date.today().year),str(datetime.date.today().month),str(datetime.date.today().day)  )
					sDB.append( aAa1 )
					#sDB.append( 'INSERT OR REPLACE INTO shows (url,title,year,img,fanart,imdbnum,plot,timestampyear,timestampmonth,timestampday) VALUES ("%s","%s","%s","%s","%s","%s","%s","%s","%s","%s")' % (  str(item_url),str(pars['title']),str(""),str(img),str(fimg),str(pars['imdbid']),urllib.quote_plus(labs['plot']),str(datetime.date.today().year),str(datetime.date.today().month),str(datetime.date.today().day)  ) )
			#if CCtoSQL==True: ## Moving visited Data from Common Cache to SQL. ##
			#		aAa2='INSERT OR REPLACE INTO visited (url,title,episode,timestampyear,timestampmonth,timestampday,img,fanart) '
			#		aAa2+='VALUES ("%s","%s","%s","%s","%s","%s","%s","%s")' % (  str(tEStItemUrl),str(r[1]),"",str(datetime.date.today().year),str(datetime.date.today().month),str(datetime.date.today().day),str(tEStLabsThumb),str(tEStLabsFanart)  )
			#		sDB.append( aAa2 )
			if len(sDB) > 0:
					debob(sDB); 
					do_database(sDB); 
