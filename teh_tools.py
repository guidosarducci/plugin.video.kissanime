### ############################################################################################################
###	#	
### # Project: 			#		teh_tools - by The Highway 2013-2015.
### # Author: 			#		The Highway
...
net = config.net
__plugin__	=	ps('__plugin__'); __authors__	=	ps('__authors__'); plugin_id		=	ps('_plugin_id'); metamind=ps('_domain_url'); _domain_url=ps('_domain_url')
### ############################################################################################################
### ############################################################################################################
import xbmc,xbmcplugin,xbmcgui,xbmcaddon,xbmcvfs,urllib,urllib2,re,os,sys,htmllib,string,StringIO,logging,random,array,time,datetime
#import copy
#try: import json
...
params=get_params(); 
ICON=os.path.join(__home__,'icon.png'); fanart=os.path.join(__home__,'fanart.jpg'); 
...
def cFL_(t,c=ps('default_cFL_color')): return '[COLOR '+c+']'+t[0:1]+'[/COLOR]'+t[1:] ### For Coloring Text ###
### ############################################################################################################
### ############################################################################################################
...
def addDirD(name,name2,url,type2,mode,iconimage,fanimage,doSorting=False,categoryA='Blank',Labels='none'):#,plot='Blank',genres='none listed',status='none',released='unknown',rating='none',others='none'):
        ...
        if doSorting==True: xbmcplugin.addSortMethod(int(sys.argv[1]), xbmcplugin.SORT_METHOD_TITLE)
        ok=xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url=u,listitem=liz,isFolder=True)
        return ok
def addDirV(name,name2,url,type2,mode,iconimage,fanimage,categoryA=''):
        ok=True; u=sys.argv[0]+"?url="+urllib.quote_plus(url)+"&mode="+str(mode)+"&name="+urllib.quote_plus(name)+"&nm="+urllib.quote_plus(name2)+"&tp="+str(type2)+"&cat="+categoryA
        liz=xbmcgui.ListItem(name,iconImage="DefaultFolder.png",thumbnailImage=iconimage)
        liz.setInfo( type="Video",infoLabels={"Title":name})
        liz.setProperty("Fanart_Image",fanimage)
        ok=xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url=u,listitem=liz,isFolder=True)
...
def download_metapack(url,dest,displayname=False):
    print 'Downloading Metapack'; print 'URL: %s'%url; print 'Destination: %s'%dest
    if not displayname: displayname=url
    dlg=xbmcgui.DialogProgress(); dlg.create('Downloading','',displayname); start_time=time.time()
    if os.path.isfile(dest):
        print 'File to be downloaded already esists'
        return True
    try: urllib.urlretrieve(url,dest,lambda nb, bs, fs: _pbhook(nb, bs, fs, dlg, start_time))
    except:
...
def download_file_prep(url,name='none',name2='none',show='none',filext='none'):
	...
	else: localpath=getset('folder-show')#__settings__.getSetting('folder-show')
...
def showkeyboard(txtMessage="",txtHeader="",passwordField=False,DefaultR=False):
	if txtMessage=='None': txtMessage=''
	keyboard=xbmc.Keyboard(txtMessage,txtHeader,passwordField); keyboard.doModal(); #("text to show","header text", True="password field"/False="show text")
	if keyboard.isConfirmed(): return keyboard.getText()
	else: return DefaultR #return False # return ''
def dialogbox_number(Header="",n='',type=0):
	...
	dlg=xbmcgui.Dialog()
	if (n==''): r=dlg.numeric(type,Header)
	else: 			r=dlg.numeric(type,Header,n)
	return r
### ############################################################################################################
def checkForPartNo(url,partInfo=''):
	url=urllib.unquote_plus(url)
	if '_part_' in urllib.unquote_plus(url):
		try: matchaptn=re.compile('_part_(.+?).').findall(url); partInfo=' - Part # '+matchaptn[0]
		except: partInfo=' - Part # '+'Unknown'
	elif '-part-' in urllib.unquote_plus(url):
		try: matchaptn=re.compile('-part-(.+?).').findall(url); partInfo=' - Part # '+matchaptn[0]
		except: partInfo=' - Part # '+'Unknown'
	elif 'part' in urllib.unquote_plus(url):
		try: matchaptn=re.compile('part(.+?).').findall(url); partInfo=' - Part # '+matchaptn[0]
		except: pass
	return partInfo
### ############################################################################################################
def sContent(content,h=int(sys.argv[1])): xbmcplugin.setContent(handle=h,content=content) #Note: content: files, songs, artists, albums, movies, tvshows, episodes, musicvideos
...
def sPluginFanart(i=_addon.get_fanart(),c1='',c2='',c3='',h=int(sys.argv[1])): i=xbmc.translatePath(i); debob({'PluginFanart':i,'h':h}); xbmcplugin.setPluginFanart(handle=h,image=i)
def aSortMeth(sM,h=int(sys.argv[1])): xbmcplugin.addSortMethod(handle=h,sortMethod=sM)
...
def page_last_get(defaultLastPage=sys.argv[0]+'?mode=0'):
  ...
  erNoFavs='XBMC.Notification([B][COLOR orange]Last Visited[/COLOR][/B],[B]You have no last_visited saved.[/B],5000,"")'
...
def myNote(header='',msg='',delay=5000,image=ICON): #'http://upload.wikimedia.org/wikipedia/commons/thumb/a/a5/US_99_%281961%29.svg/40px-US_99_%281961%29.svg.png'
...
def qBaRt(url): 
	...
	if len(t) > 0: 
		...
		temp_file=xbmc.translatePath(os.path.join(addon_profile_path,'temp.html.__.txt'))
		if (_debugging==True) and (_testing==True):
			try: _SaveFile(temp_file,html)
			except: pass
	try: xbmc.sleep(int(addst("ResTimer")))
	except: pass
def CheckUrlForRedirect(url,headers={},proxy='',User_Agent='',cookie_file='',load_cookie=False,save_cookie=False,html=''):
	return url

def httpHEADER(url,headers={}):
	#try:
		req=HeadRequest(url)
		req.add_header('User-Agent',net._user_agent)
		try: _accept=net._accept
		except: _accept='text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'
...
def nURL(url,method='get',form_data={},headers={},html='',proxy='',User_Agent='',cookie_file='',load_cookie=False,save_cookie=False,login=False,compression=True):
	if url=='': return ''
	if not login:
		if Can_Login() and not Check_Login():
			_T__F_=KA_Login()
	...
	while (AntiTag in html):
		...
		if   method.lower()=='get':
			try: 
				...
				html=net.http_GET(url,headers=headers,compression=compression).content
		...
		elif method.lower()=='redirect':
			#try: html=net2.http_HEAD(url,headers=headers).content
			#try: html=net2.http_HEAD(url,headers=headers).getheaders()
			#try: html=net2.http_HEAD(url,headers=headers).geturl()
			#try: html=urllib2.urlopen(url).geturl()
			
			html=net2.http_HEAD2(url,headers=headers).geturl()
...
def refresh_meta(video_type,old_title,imdb,alt_id,year,new_title=''):
	...
	if index==0: 
		labsTitle,labsID,labs=refresh_meta_manual(video_type,old_title,imdb,alt_id,year)
		return labsTitle,labsID,labs
	elif index > -1:
		...
		try: debob(search_meta[index-1])
		except: pass
		__metaget__.update_meta(video_type,old_title,imdb,year=year); 
		#xbmc.executebuiltin('Container.Refresh')
		##
		#GRABMETA(name,types,year=None,imdb_id='',tmdb_id='',overlay=6)
		labsTitle=search_meta[index-1]['title']
		labsID=search_meta[index-1]['imdb_id']
		labs=GRABMETA(labsTitle,video_type,imdb_id=labsID)
		return labsTitle,labsID,labs
...
def refresh_meta_manual(video_type,old_title,imdb,alt_id,year):
	...
	if keyboard.isConfirmed():
		...
		labsTitle,labsID,labs=refresh_meta(video_type,old_title,imdb,alt_id,year,search_string)
		return labsTitle,labsID,labs
	else:
		return '','',''
...
def getGitMsg(user="HIGHWAY99",project="repository.thehighway"):
	DD={'sha':'','date':'','message':'','user':'error'}
	CommitUrl='http://api.github.com/repos/'+user+'/'+project+'/commits?per_page=1&sha=master'
	#https://api.github.com/repos/HIGHWAY99/repository.thehighway/commits?per_page=1&sha=master
	try:
		deb('comments',CommitUrl)
		html=nolines(nURL(CommitUrl)); r={}; 
		_SaveFile(os.path.join(addonPath,'commits.txt'),html)
		if len(html)==0: return DD
		...
		if hasattr(e,'code'):
			if   hasattr(e,'reason'): debob({'code':e.code,'reason':e.reason})
			elif hasattr(e,'message'): debob({'code':e.code,'message':e.message})
			elif hasattr(e,'msg'): debob({'code':e.code,'msg':e.msg})
			else: debob({'code':e.code})
		else: debob({'error':'with getGitMsg()'})
		return DD
	except: return DD
	##
...
def DoSeriesLabelFormatting(LabelFormat='',DefaultT='',LInfo='',imInfo='',FoundItK=False,Title='',name='',animename='',animetype='',labs={},pars={}):
	#try:
		UseLabelFormat=tfalse(addst("use-series-labels-formatting","false"))
		if UseLabelFormat==False: return DefaultT
		else:
			if len(LabelFormat)==0: LabelFormat=addst("text-series-labels-format","[COLOR %Watched%]@[/COLOR]  %SeriesTitle%")
			if len(LabelFormat)==0: LabelFormat="[COLOR %Watched%]@[/COLOR]  %SeriesTitle%"
			LInfo=str(LInfo); imInfo=str(imInfo); 
			...
			if ('</a>' in LInfo): 
				try: (LatestUrl,LatestName,LatestNumber)=re.compile('<a href="(/'+ps('common_word')+'/.+?)">\s*\n*\s*(.+?(\d*))\s*\n*\s*</a>').findall(LInfo)[0]; LatestUrl=_domain_url+LatestUrl
...
def EncodeUrlB64n(link):
	try: return link.encode('base-64').replace('\n','')
	except: return link


### ############################################################################################################

def ArtworkCaching(iUrl,iPath=addon_profile_path,iData=''):
	try:
		if '|' in iUrl: iUrl2=iUrl.split('|')[0]
		else: iUrl2=''+iUrl
		if '/' in iUrl2: iUrl3=iUrl2.split('/')[-2]+iUrl2.split('/')[-1]
		else: iUrl3=''+iUrl2
		
		#iFilename='zimg_'+EncodeUrlB64n(iUrl3)+'.png'
		iFilename='zimg_'+str(iUrl3) #+'.png'
		iFilenameWP=os.path.join(iPath,iFilename)
		debob([iFilename,iFilenameWP,iUrl,iUrl2,iUrl3])
		if isFile(iFilenameWP):
			debob('Found artwork: "'+iFilenameWP+'"')
			return iFilenameWP
		##
		cookie_file=_cookie_file
		if isFile(cookie_file)==True: html_=nURL(iUrl,headers={'Referer':_domain_url},cookie_file=cookie_file,load_cookie=True,save_cookie=True,compression=False)
		else: html_=nURL(iUrl,headers={'Referer':_domain_url},proxy=Proxy,cookie_file=cookie_file,save_cookie=True,compression=False)
		debob({'length':len(html_)})
		debob(html_)
		if len(html_) > 0:
			try:
		...
		#iUrlWithHeaders=xbmc.translatePath(net.url_with_headers(iUrl2))
		#urllib.urlretrieve(iUrlWithHeaders,iFilenameWP)
		#return iFilenameWP
